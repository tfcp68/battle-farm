...
2. The Player has on option to acquire one **Card** from the _Market_, if they have enough **Coins**. The **Coins** are then discarded. The cost of **Card** is defined by its rarity.
...
4. The Player can play any number of **Cards** one by one. All the effects of the **Card** must be applied before the next **Card** is played 
    - To play a **Crop Card**, the Player should Plant it into an empty **Bed**, setting a _Ripe Timer_ on it, depending on the **Card** rarity.
...
5. The Player can use 1 **Fertilizer** to reduce _Ripe Timer_ on any **Crop** in their **Beds** by 1 turn per **Crop**. If _Ripe Timer_ is reduced to zero on any given **Crop**, it is immediately harvested, yielding **Coins**, but the emptied **Bed** can't be planted the same turn.
6. If by the end of their turn the Player has reached the Limit of **Coins**, the current turn becomes the last one for all the following Players, and the Game ends afterwards 

The Limit of **Coins** is set at the beginning of the Game depending on number of Players, and is calculated as
> 60 + `Total sum of Crop Card values in Deck` / (1 + `Total number of Players`) rounded up

For instance, if a 1v1 Game is started with standard Deck, the **Coin** Limit would be 60 + 418/3 ~= 200
...
**Cards** are the main expendable source of the Game. There is a set quantity of each **Card** in the _Deck_, and, once the _Deck_ is over, the Game is over too, if it was not finished eariler. Every Player gets a given number of **Cards** in the beginning of the Game, depending on their _Class_, and 6 more are placed at _Market_ their face up. All the rest reside in _Deck_ until required by certain actions. So at every moment of the Game every Player sees only their hand and the _Market_, as well as planted **Crops**, which previously were **Crop Cards**.
...
After the given number of turns, the **Crop** automatically dissapears from **Bed**, granting **Coins** to the owning Player. The **Crop Value** of the **Crop** and its _Ripe Timer_ can be modified during growth with other **Cards**. Combination of this params and acquisition cost is calculated as Gold Per Turn (`GPT`):
> `Gold Per Turn` = ( `Crop Value` - `Seed Cost` ) / `Ripe Timer`
...
| Onions        | Green     | Common   | 8        | `Onion Ring`: when Planted, gain 1 **Fertilizer** for every other growing `Onion`               | 1 | 3   | 1 | 2     |
...
| Eggplant      | Red       | Rare     | 4        | `Eggplant Emoji`: when Planted, pick a **Crop**. It loses 1 Value and has its _Ripe Timer_ increased by 1   | 3 | 9   | 3 | 2     | 
...
| Pumpkins      | Yellow    | Epic     | 2        | `Trick or Treat`: when Planted, pick a Player. They give you 3 **Coins** for every **Fertilizer** they have | 5 | 15  | 4 | 2.5   |
| Oranges       | Red       | Epic     | 2        | `Sweet and Sour`: when Planted and Harvested, all other growing **Crops** gain 1 Value | 5 | 15  | 4 | 2.5   |
| Grapes        | Green     | Epic     | 2        | Grapevine            | 5 | 15  | 4 | 2.5   |
| Lignonberry   | Yellow    | Mythic   | 1        | Berry Blitz           | 8 | 21  | 5 | 2.6   |
| Strawberry    | Red       | Mythic   | 1        | Ripe for the Picking  | 8 | 23  | 5 | 3     |
| Blueberry     | Green     | Mythic   | 1        | Blueberry Boom        | 8 | 22  | 5 | 2.8   |
