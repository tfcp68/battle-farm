...
return [EventBus, automatas];
  ...
  "GAME_JOIN_PENDING": "GAME_JOIN_PENDING"
			...
			export type TActionsMenuSubmodeAutomata = keyof typeof actionsMap;
...
export class MenuSubmodeAutomata extends GenericAutomata {

    static id = 'MenuSubmodeAutomata_1774195154806';
    static actions = actionsMap;
    static states = statesMap;
    static getState = (state: keyof typeof statesMap) => statesDictionary[state];
    static hasState = (instance: MenuSubmodeAutomata, state: keyof typeof MenuSubmodeAutomata.states) => instance.state === MenuSubmodeAutomata.getState(state);
    ...
    static createAction = (action: keyof typeof actionsMap, payload:any) => {
		const actionId = MenuSubmodeAutomata.getAction(action);
		return {
			action: actionId,
			payload,
		}
...
export default MenuSubmodeAutomata;
			const epoch = { val: 1 };
const incrementEpoch = () => { epoch.val++ };
const getEpoch = () => epoch.val;
			const internals = {
	...internalFunctions,
	"currentStateId": internalFunctions.currentStateId(MenuSubmodeAutomata),
	"currentStateName": internalFunctions.currentStateName(MenuSubmodeAutomata, statesDictionary),
	"currentActionId": internalFunctions.currentActionId(MenuSubmodeAutomata),
	"currentActionName": internalFunctions.currentActionName(MenuSubmodeAutomata, actionsDictionary),
	"currentCycle": internalFunctions.currentCycle(MenuSubmodeAutomata),
