  ...
  "~~~END~~~": 1644281759,
  "TRADING": 349384447,
  "CONFIRM_TRADE": 329668325,
  "ACCEPTING_TRADE": 1566252671
}
export const actionsDictionary = {
  ...
  "CONFIRM_DEAL": 148692651
}
export const eventDictionary = {
  "browseMarket": 695113574,
  "enableTargetMode": 251799735,
  "forceEndPhase": 223655275,
  "disableTargetMode": 655528996,
  "startMarketAcquisition": 68914833,
  "completeMarketAcquisition": 1286312584
}
...
eventAdapter.addEventEmitter(
		...
		const eventsToEmit = [
			{
			event: eventDictionary["browseMarket"],
			meta: {
...
eventAdapter.addEventEmitter(
				statesDictionary["TRADING"], 
				({ state, context }) => {
		const eventsToEmit = [
			{
			event: eventDictionary["enableTargetMode"],
			meta: {
				CARD_MARKET: (function(){
			const boundValue = (function(){
						if(context !== null && context['CARD_MARKET'] !== undefined && context['CARD_MARKET'] !== null) {
							return context['CARD_MARKET']
						}
							else {
								return null
							}
...
eventAdapter.addEventEmitter(
				statesDictionary["CONFIRM_TRADE"], 
				({ state, context }) => {
		const eventsToEmit = [
			{
...
eventAdapter.addEventEmitter(
				statesDictionary["ACCEPTING_TRADE"], 
				({ state, context }) => {
		const eventsToEmit = [
			{
			event: eventDictionary["startMarketAcquisition"],
			meta: {
...
eventAdapter.addEventListener(
        eventDictionary["completeMarketAcquisition"], 
        ({ event, meta }) => {
...
return [EventBus, automatas];
  ...
  "ACCEPTING_TRADE": "ACCEPTING_TRADE"
			...
			export type TActionsShoppingAutomata = keyof typeof actionsMap;
	...
	349384447: (prevContext, payload, functionDictionary, automata) => {

				return prevContext
			},
	329668325: (prevContext, payload, functionDictionary, automata) => {

				return prevContext
			},
	1566252671: (prevContext, payload, functionDictionary, automata) => {

				return prevContext
			}
	}
			const predicates = {
		2242516: { 1546481415: (prevContext, payload, functionDictionary) => {
...
329668325: { 1167674855: (prevContext, payload, functionDictionary) => {
...
1566252671: { 1546481415: (prevContext, payload, functionDictionary) => {
	...
	2242516: {
			...
			,
				...
				1546481415: {
					state: [108966002,349384447],
predicate: predicates[2242516][1546481415]
				}
	...
	329668325: {
			...
			,
				...
				1167674855: {
					state: [108966002,349384447],
predicate: predicates[329668325][1167674855]
				}
	...
	1566252671: {
			...
			,
				...
				1546481415: {
					state: [108966002,349384447],
predicate: predicates[1566252671][1546481415]
				}
			
	},}
			
export class ShoppingAutomata extends GenericAutomata {

    static id = 'ShoppingAutomata_1774195154833';
    static actions = actionsMap;
    static states = statesMap;
    static getState = (state: keyof typeof statesMap) => statesDictionary[state];
    static hasState = (instance: ShoppingAutomata, state: keyof typeof ShoppingAutomata.states) => instance.state === ShoppingAutomata.getState(state);
    ...
    static createAction = (action: keyof typeof actionsMap, payload:any) => {
		const actionId = ShoppingAutomata.getAction(action);
		return {
			action: actionId,
			payload,
		}
...
export default ShoppingAutomata;
			const epoch = { val: 1 };
const incrementEpoch = () => { epoch.val++ };
const getEpoch = () => epoch.val;
			const internals = {
	...internalFunctions,
	"currentStateId": internalFunctions.currentStateId(ShoppingAutomata),
	"currentStateName": internalFunctions.currentStateName(ShoppingAutomata, statesDictionary),
	"currentActionId": internalFunctions.currentActionId(ShoppingAutomata),
	"currentActionName": internalFunctions.currentActionName(ShoppingAutomata, actionsDictionary),
	"currentCycle": internalFunctions.currentCycle(ShoppingAutomata),
