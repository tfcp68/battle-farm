...
function _getCardIDByType(
	card: TCardDescriptor<TCardType.CROP>
): TCropId | null;
function _getCardIDByType<T extends TCardType>({
	type,
	id,
}: TCardDescriptor<T>) {
	if (type === TCardType.ACTION)
		return Object.keys(ActionCardId)[
			Object.values(ActionCardId).indexOf(id)
		] as TCardId<TCardType.ACTION>;
	if (type === TCardType.CROP)
		return Object.keys(CropCardId)[
			Object.values(CropCardId).indexOf(id)
		] as TCropId;
	return null;
}
