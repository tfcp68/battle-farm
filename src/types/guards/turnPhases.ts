...
export const isFertilizeSubphase =
	...
	(currentSubphase: any): currentSubphase is T =>
		null === targetSubphase
			? Object.values(TFertilizePhase).includes(currentSubphase) &&
			  Number.isFinite(currentSubphase)
...
export const isFertilizeContext =
	...
	(
		...
		null === targetSubphase
			? Object.values(TFertilizePhase).includes(
					currentContext?.subPhase
...
export const isFertilizeAction =
	...
	(currentAction: any): currentAction is T =>
		null === targetAction
			? Object.values(TFertilizeAction).includes(currentAction) &&
			  Number.isFinite(currentAction)
	...
	(
		...
		null === targetAction
			? Object.values(TFertilizePhase).includes(currentPayload?.action) &&
			  Number.isFinite(currentPayload?.action)
