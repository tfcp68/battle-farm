...
export default class AuthModel extends BaseModel {
	async register(nickname: string, password: string) {
		...
		const { data: existing } = await this.db.from('players').select('*').eq('user_id', user.id).maybeSingle();

		if (existing) {
			return { user, player: { playerId: existing.id, nickname: existing.nickname } };
		}

		const { data, error } = await this.db.from('players').insert({ nickname, user_id: user.id }).select().single();
	...
	async signIn(nickname: string, password: string) {
		...
		const { data: player, error } = await this.db.from('players').select('*').eq('user_id', user.id).single();
		if (error) throw error;
	...
	async getCurrentPlayer() {
		...
		const { data: player } = await this.db.from('players').select('*').eq('user_id', user.id).maybeSingle();
