...
// Example (dev): VITE_AUTH_BASE_EMAIL_LOCAL=mygmailname, VITE_AUTH_BASE_EMAIL_DOMAIN=gmail.com
...
export async function signUpWithNickname(nickname: string, password: string) {
	...
	if (error) {
		if (error.code === 'email_address_invalid') {
			throw new Error(`Supabase rejected "${email}". Check VITE_AUTH_BASE_EMAIL_LOCAL and VITE_AUTH_BASE_EMAIL_DOMAIN (use a real mailbox domain, e.g. gmail.com).`);
		}
		throw error;
	}
	return data;
